	...
	$gender = array('Male'=>'Male','Female'=>'Female');
		$primary_director = array('Yes'=>'Yes','No'=>'No');
	?>
	<style>
	   .budget_proposal {
	   width: 100%;
...
if(upload1>0) {
	...
	<?php 
	if(in_array(155,$decode) && !empty($decode))
	{									
	?>
	var upload155 = document.getElementById("upload155").files.length;
	if(upload155>0) {
		var file_size155 = $('#upload155')[0].files[0].size;
						size155 = 5*1000*1000;
						if(file_size155>size155) {
							error1 = 1;
							$("#size_error155").show();
						} else {
							$("#size_error155").hide();
	...
	<?php 
	if(in_array(133,$decode) && !empty($decode))
	{									
	?>
	var upload133 = document.getElementById("upload133").files.length;
	if(upload133>0) {
		var file_size133 = $('#upload133')[0].files[0].size;
						size133 = 5*1000*1000;
						if(file_size13>size13) {
							error1 = 1;
							$("#size_error133").show();
				   ...
				   <form role="form" id="quickForm1" method="POST" action="{{ url('/forms/save_form1') }}" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
						 <div class="form-group col-md-6">
								...
								<div class="d-md-flex align-items-center">
									...
									<div class="form-row col">
										<div class="col">
											...
											<input type="date" value="{{isset($data['start_date'])?$data['start_date'] : ''}}" name="start_date" class="form-control required">
									...
									<div class="form-row col">
										<div class="col">
											...
											<input type="date" value="{{isset($data['end_date'])?$data['end_date'] : ''}}" name="end_date" class="form-control required">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
									</div>
							...
							<h3>Course Information</h3>
							 <div class="card sport_card">
								...
								<div class="col-md-6">
									<div class="form-row col">
										<div class="col">
											<label><i class="fas fa-circle"></i>Course Location</label>
											<input type="text" class="form-control required" name="course_location" value="{{isset($data['course_location'])?$data['course_location']:''}}">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
									</div>
									...
									<div class="form-row col">
										<div class="col">
											<label><i class="fas fa-circle"></i> ?Online or In Person</label>
											</div>
											<div class="col">
											 <input class="form-check-input" type="checkbox" id="inlineCheckbox1" name="in_person" value="Active" {{isset($data['in_person']) && $data['in_person']  == "Active" ? 'checked' : ''}} />
											  <spacer><label class="form-check-label" for="inlineCheckbox1">Person</label>
											<div class="required_error">&nbsp;</div>
										</div>
										<div class="col">
											
											 <input class="form-check-input" type="checkbox" id="inlineCheckbox1" name="in_person" value="Active" {{isset($data['in_person']) && $data['in_person']  == "Active" ? 'checked' : ''}} />
											   <spacer><label class="form-check-label" for="inlineCheckbox1">Online</label>
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
									</div>
								...
								<p>
									If your NOC has several PDs please include the primary PD in the field below, you can include other PDs that may take part as
									facilitators in the field below “Other PD”. We also invite your NOG to review the list and communicate to OS any updates needed
								</p>
								</div>
								<br>
								<h3 style="color: #1b85c3;font-size: 22px;font-weight: 600;">Primary Programme Director</h3>
								<div class="card sport_card">
									<div class="d-md-flex align-items-center">
										<div class="form-row col">
										<div class="col">
											<label>Name of Primary Programme Director</label>
											<input type="text" class="form-control" name="primary_programme" value="{{isset($data['primary_programme'])?$data['primary_programme']:''}}">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
									</div>
								...
								<br>
								<h3 style="color: #1b85c3;font-size: 22px;font-weight: 600;">Other Programme Director(s)</h3>
								<div class="card sport_card">
								 <?php
							 if(!empty($data))
							 {
							$num=count($data['secondary_programme']);
							for($i=0;$i<$num;$i++)
								{
								?>	
								<div class="add_director">
								<div class="col add_director"><input type="text" class="form-control" name="secondary_programme[]" value="<?php if(isset($data['secondary_programme'][$i]) && !empty($data['secondary_programme'][$i])) echo $data['secondary_programme'][$i] ?>"><div class="required_error">&nbsp;</div></div>
								</div>
							 <?php } } else { ?>
							 <div class="add_director">
							 ...
							 <?php } ?>
								<div class="col-md-12">
									<button type="button" name="" class="btn btn-success add_field_button">Add</button>
								</div>
								</div>
								...
								<div class="col-md-6">
								<div class="col">
								<p>lf the Programme Director name doesn't appear in the list above,please click on "No" below and type the name of the ProgrammeDirector in the new field </p>
								<select class="form-control required" name="director" aria-label="Default select example">
								<option value=""></option>  
								<?php foreach($primary_director as $vals){ ?>
								<option <?php if(isset($data['director']) && $data['director']==$vals) echo 'selected' ?> value="<?php echo $vals ?>"><?php echo $vals ?></option>
								<?php } ?>
							</select>
								<div class="required_error">&nbsp;</div>
										</div>
							</div>	
							<h3 style="color: #1b85c3;font-size: 22px;font-weight: 600;">Participants</h3>
							<div class="col-md-12">
							<div class="form-row col">
										<div class="col">
											<label>Target number of participants</label>
											<input type="text" class="form-control  " name="participants" value="{{isset($data['participants'])?$data['participants']:''}}">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
							</div>
							...
							<div class="form-row col">
										<div class="col">
											<label><i class="fas fa-circle"></i>?Who are you targeting as course participants (type of organisations, functions, etc.)</label>
											<input type="text" class="form-control " name="targeting_course" value="{{isset($data['targeting_course'])?$data['targeting_course']:''}}">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
							</div>
							...
							<div class="form-row col">
										<div class="col">
											<label><i class="fas fa-circle "></i>?What are the pre-requisites for course participants</label>
											<input type="text" class="form-control" name="course_participants" value="{{isset($data['course_participants'])?$data['course_participants']:''}}">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
							</div>
<p>
Please make sure that the timetable reflects the distribution of time between the different activities that compose the training block.
The programme should also clearly indicate the different chapters covered during the course, the time dedicated to the participants
case study presentations and the impact presentations </p>
							<br>
								 <h3 style="color: #1b85c3;font-size: 22px;font-weight: 600;">Detailed programme of the course and timetable of each session</h3>
								<div class="card sport_card">

                                      <tr>
                                    <td style="width: 80%;">Image1</td>
                                    <td>
                                        <div class="row">
                                            @if(!empty($attachments) && isset($attachments['attachment']) && isset($attachments['attachment']['curriculum_vitae2']))
                                                <a target="_blank" href="{{url('upload/form_document/'.$dataUserApplications['user_id'].'/'.$attachments['attachment']['curriculum_vitae2'])}}">{{$attachments['attachment']['curriculum_vitae2']}}</a>
                                            @endif
                                            ...
                                            @if (( \Auth::user()->hasAnyRole(['User']) && !isset($data) ) || (\Auth::user()->hasAnyRole(['User']) && (isset($dataUserApplications) && ($dataUserApplications['status'] == 'request not completed' || $dataUserApplications['status'] == 'not submitted'))))
                                                <label for = "upload155">File Uploade</label>
                                                <input class="custom-file-input" type="file" id="upload155" name="attachment[curriculum_vitae2]">
 ...
 <tr>
                                    <td style="width: 80%;">Image2</td>
                                    <td>
                                        <div class="row">
                                            @if(!empty($attachments) && isset($attachments['attachment']) && isset($attachments['attachment']['curriculum_vitae1']))
                                                <a target="_blank" href="{{url('upload/form_document/'.$dataUserApplications['user_id'].'/'.$attachments['attachment']['curriculum_vitae1'])}}">{{$attachments['attachment']['curriculum_vitae1']}}</a>
                                            @endif

                                            @if (( \Auth::user()->hasAnyRole(['User']) && !isset($data) ) || (\Auth::user()->hasAnyRole(['User']) && (isset($dataUserApplications) && ($dataUserApplications['status'] == 'request not completed' || $dataUserApplications['status'] == 'not submitted'))))
                                                <label for = "upload133">Image2</label>
                                                <input class="custom-file-input" type="file" id="upload133" name="attachment[curriculum_vitae1]">
													<span style="display:none;color: red;font-size: 13px;" id="size_error1">@lang('custom.file_size')</span>
                                            @endif
                                        </div>
                                    </td>
			              ...
			              <h3>Manuals</h3>
							 <div class="sport">
							<h3>Please indicate the number and language of manuals required</h3>
							 <div class="card sport_card">
								<div class="d-md-flex align-items-center">
									<div class="row">
									<p><b>Manuals</b>: the official Managing Olympic Sport Organisations manual is available in English, French and Spanish. In the sectionbelow, we invite you to please indicate the number of manuals necessary for the course. </p>
									
									<div class="col-md-4 col-sm-12">
									<div class="form-row col">
										<div class="col">
											<label>Number of manuals in English Number</label>
											<input type="number" class="form-control" name="english_number" value="{{isset($data['english_number'])?$data['english_number']:''}}">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
									</div>
									...
									<div class="form-row col">
										<div class="col">
											<label>Number of manuals in French Number</label>
											<input type="number" class="form-control" name="french_number" value="{{isset($data['french_number'])?$data['french_number']:''}}">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
									</div>
									...
									<div class="form-row col">
										<div class="col">
											<label>Number of manuals in Spanish</label>
											<input type="number" class="form-control" name="spanish_number" value="{{isset($data['spanish_number'])?$data['spanish_number']:''}}">
											<div class="required_error">&nbsp;</div>
										</div>
									</div>
									</div>
									
									<p><b>Manuals in other languages (translated by NOCs) are avallable under the Resources tab (NOC Management & Knowledge Sharing - Advanced Sports Administrators Courses). </b></p>
								</div>
            ...
            <div class="card-body">
               ...
               <p>The course subsidy may be used to cover expenses directly related to the organisation of the course, such as Programme
						Director/Faciltator(s)/Expert(s) fees, conference faciity/equipment rental, coffee breaks, printing/translation of documents,
						transport if needed, office supplies. In case of other expenses, please explain why they are needed. .</p>
			   <div class="required_error">&nbsp;</div>
...
$(add_button).click(function(e){
	...
	if(total_fields < max_fields){
		$(wrapper).append('<div class="col add_director"><input type="text" class="form-control" name="secondary_programme[]"><div class="required_error">&nbsp;</div></div>');
		 $('.textarea').summernote();
	}
});
...
<script type="text/javascript">
    ...
    var max_fields1      = 20;
var wrapper1         = $(".add_file"); 
var add_button1      = $(".add_file_button");
var remove_button1   = $(".remove_field_button");

$(add_button1).click(function(e){
	e.preventDefault();
	var total_fields1 = wrapper1[0].childNodes.length;
	if(total_fields1 < max_fields1){
		$(wrapper1).append('<div class="d-md-flex align-items-center  "><div class="form-row col"><div class="col"><label><i class="fas fa-circle"></i>File upload</label><input type="file" class="form-control" name="file_upload[]" value=" "><div class="required_error">&nbsp;</div></div></div></div>');
		 $('.textarea').summernote();
	}
});
